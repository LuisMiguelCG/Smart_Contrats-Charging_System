...
contract ChargingSystem {
    ...
    function addNewUser(uint256 _time) internal returns (bool) {
        ...
        UserToCharger[msg.sender] = Charger(msg.sender, idCharger, block.timestamp, block.timestamp + _time);
        ...
        uint256 minUserStop = minUser != address(0x0) ? UserToCharger[minUser].stopTime : 0;
        uint256 nextMinUserStop = nextMinUser != address(0x0) ? UserToCharger[nextMinUser].stopTime : 0;
    ...
    function refreshBusyChargers() internal returns (bool) {
        ...
        Charger memory minUser =
            minTimeUser.addr != address(0x0) ? UserToCharger[minTimeUser.addr] : Charger(address(0x0), 0, 0, 0);
